...
#define SECTOR_SIZE     g_SECTOR_SIZE
...
unsigned int g_SECTOR_SIZE = g_PAGE_SIZE;
...
int main(int argc, char* argv[])
{
    ...
    for(i = 1; (int)i < argc; i++)
    {
        ...
        else if(strcmp(argv[i], "-sectorsize") == 0 || strcmp(argv[i], "--sectorsize") == 0)
        {
            if((int)i + 1 < argc)
            {
                i++;
                if(strncmp(argv[i], "0x", 2) == 0 || strncmp(argv[i], "0X", 2) == 0)
                    g_SECTOR_SIZE = strtoul(argv[i], NULL, 16);
                else
                    g_SECTOR_SIZE = strtoul(argv[i], NULL, 10);
                printf("Sector size set to: %d bytes\n", g_SECTOR_SIZE);
            }
            else
            {
                printf("ERROR: -sectorsize requires a value\n");
                return -1;
            }
        }
        ...
        else if(strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "/?") == 0)
        {
            ...
            printf("  -sectorsize <size>: Flash sector erase size in bytes\n");
            printf("                      Default: %d bytes\n", PAGE_SIZE_DEFAULT);
    ...
    if(g_ENABLE_VERIFY)
    {
        ...
        /* Erase the 2nd sector only and verify: sector0 & sector2 keep data, sector1 becomes 0xFF */
        if(g_TEST_PAGES * g_PAGE_SIZE >= g_SECTOR_SIZE * 3)
        {
            printf("Performing single-sector erase test...\n");
            EraseSectors(g_TEST_BASE / g_SECTOR_SIZE + 1, 1);

            /* Clean the buffer and read back all test region */
            memset(buf, 0xCC, g_TEST_PAGES * g_PAGE_SIZE);
            ReadPages(buf, g_TEST_BASE / g_PAGE_SIZE, g_TEST_PAGES);

            isErr = 0;
            /* sector 0: original data (idx & 0xFF) */
            for(idx = 0; idx < g_SECTOR_SIZE; idx++)
            {
                if(buf[idx] != (idx & 0xFF))
                {
                    printf("ERROR: Sector0 data mismatch at %d, expected 0x%02X, got 0x%02X\n",
                           idx, (idx & 0xFF), buf[idx]);
                    isErr = 1;
                    break;
                }
            }
            /* sector 1: erased -> 0xFF */
            for(idx = g_SECTOR_SIZE; idx < g_SECTOR_SIZE * 2; idx++)
            {
                if(buf[idx] != (unsigned char)0xFF)
                {
                    printf("ERROR: Sector1 not erased at %d, got 0x%02X\n", idx, buf[idx]);
                    isErr = 1;
                    break;
                }
            }
            /* sector 2: original data */
            for(idx = g_SECTOR_SIZE * 2; idx < g_SECTOR_SIZE * 3; idx++)
            {
                if(buf[idx] != (idx & 0xFF))
                {
                    printf("ERROR: Sector2 data mismatch at %d, expected 0x%02X, got 0x%02X\n",
                           idx, (idx & 0xFF), buf[idx]);
                    isErr = 1;
                    break;
                }
            ...
            printf("Single-sector erase test passed.\n");
        }
        else
        {
            printf("Skip single-sector erase test (test size %u < 3 sectors of %u).\n",
                   g_TEST_PAGES * g_PAGE_SIZE, g_SECTOR_SIZE);
        }
