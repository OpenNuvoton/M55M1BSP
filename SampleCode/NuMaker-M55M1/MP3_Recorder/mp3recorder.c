...
void Recorder_Init(void)
{
    ...
    I2S_Open(I2S_PORT, I2S_MODE_SLAVE, REC_SAMPLE_RATE, I2S_DATABIT_16, REC_FORMAT, I2S_FORMAT_I2S);

    NVIC_EnableIRQ(I2S_IRQ);
    NVIC_SetPriority(I2S_IRQ, 4);

    /* Set MCLK and enable MCLK */
    I2S_EnableMCLK(I2S_PORT, 12000000);

    /* NAU8822 will store data in left channel */
    I2S_SET_MONO_RX_CHANNEL(I2S_PORT, I2S_MONO_LEFT);
    ...
    I2S_EnableInt(I2S_PORT, I2S_IEN_RXTHIEN_Msk);
#else
    /* Open I2S0 interface and set to slave mode, audio format, I2S format */
    SPII2S_Open(SPI_PORT, SPII2S_MODE_SLAVE, REC_SAMPLE_RATE, SPII2S_DATABIT_16, SPII2S_MONO, SPII2S_FORMAT_I2S);

    /* Set MCLK and enable MCLK (MCLK can provide clock source to NAU8822) */
    SPII2S_EnableMCLK(SPI_PORT, 12000000);

    //NAU8822 will store data in left channel
    SPII2S_SET_MONO_RX_CHANNEL(SPI_PORT, SPII2S_MONO_LEFT);
    ...
    /* Enable RX threshold level interrupt */
    SPII2S_EnableInt(SPI_PORT, SPII2S_FIFO_RXTH_INT_MASK);
...
void MP3Recorder(void)
{
    ...
    {
        pu8Data = shine_encode_buffer_interleaved(s, (int16_t *)(SPIM_HYPER_DMM0_SADDR + u32BuffPos), (int *)&i32Written);
