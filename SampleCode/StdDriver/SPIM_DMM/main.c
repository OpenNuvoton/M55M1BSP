...
int dma_rw_patten(SPIM_T *spim, int is4ByteAddr, uint32_t u32RdCmd, uint32_t WrCmd)
{
    ...
    SPIM_EraseBlock(spim, TEST_BLOCK_ADDR, is4ByteAddr, OPCODE_BE_64K, SPIM_BITMODE_1, SPIM_OP_ENABLE);
    printf("done.\n");
    ...
    */
    ...
    {
        /* Fill the buffer with 0s before reading the flash data */
        memset(gau8buff, 0, BUFFER_SIZE);

#if (NVT_DCACHE_ON == 1)
        /* If the data cache is enabled, invalidate the cache for the buffer to ensure data consistency */
        SCB_InvalidateDCache_by_Addr((uint32_t *)&gau8buff, sizeof(gau8buff));
#endif

        /* Read data from the SPI flash */
        SPIM_IO_Read(SPIM_PORT, TEST_BLOCK_ADDR + offset, is4ByteAddr,
        ...
        {
            if (pData != NULL && i + offset < FLASH_BLOCK_SIZE)
            {
                if (*pData != 0xFFFFFFFF)
                {
                    printf("FAILED!\n");
                    printf("\tFlash address 0x%x, read 0x%x!\n",
                           TEST_BLOCK_ADDR + i + offset,
                           *pData);
    ...
    */
    ...
    for (offset = 0; offset < FLASH_BLOCK_SIZE; offset += BUFFER_SIZE)
    {
        ...
        /* Fill the buffer with sequential data before writing it to the SPI flash */
...
#if (NVT_DCACHE_ON == 1)
        /* clean the data cache for the temporary buffer before writing to the SPI flash */
        SCB_CleanDCache_by_Addr((uint32_t *)&gau8buff, sizeof(gau8buff));
#endif
        /* Write data from the buffer to the SPI flash using DMA mode */
        SPIM_DMA_Write(spim, TEST_BLOCK_ADDR + offset, is4ByteAddr, BUFFER_SIZE, gau8buff, WrCmd);
    }
...
int dmm_read(SPIM_T *spim, int is4ByteAddr, uint32_t u32RdCmd, uint32_t WrCmd)
{
    ...
    for (offset = 0; offset < FLASH_BLOCK_SIZE; offset += BUFFER_SIZE)
    {
        /* Clean the buffer with 0s before reading the SPI flash data */
        memset(gau8buff, 0, BUFFER_SIZE);
        ...
        SCB_InvalidateDCache_by_Addr((uint32_t *)&gau8buff, sizeof(gau8buff));
#endif
        /* Copy the data from the direct map mode address to the buffer */
        memcpy(gau8buff, (uint8_t *)(u32DMMAddr + TEST_BLOCK_ADDR + offset), BUFFER_SIZE);
        ...
        {
            printf("FAILED!\n");
            printf("\tNull pointer reference for pData!\n");
            SPIM_ExitDirectMapMode(spim);
            return SPIM_ERR_FAIL;
        }

        /* Verify the data read from the SPI flash with DMM read */
        for (i = 0; i < BUFFER_SIZE; i += 4, pData++)
        {
            /* The data read from the SPI flash should match the expected value */
