...
/**
  ...
  *
  * @param    pu8Buff     Pointer to buffer to be dumped
  * @param    i32nSize    Number of bytes to be dumped
  *
  * @return   None
  *
    ...
    for (u32Idx = 0; u32Idx < i32nSize; u32Idx += u32nPerLine)
    {
        ...
        for (u32i = u32Idx; u32i < u32Idx + u32nPerLine && u32i < i32nSize; u32i++)
        {
            if (pu8Buff[u32i] >= 0x20 && pu8Buff[u32i] < 127)
                ...
                printf("."); /* Dump a dot if not printable */
        }
...
/**
  *  DMA read/write data to/from SPI flash
  */
int dma_read_write(int is4ByteAddr, uint32_t u32RdCmd, uint32_t WrCmd)
{
    uint32_t u32i = 0, u32Offset = 0; /* variables */
    uint32_t *pu32Data = NULL;

    /* Enable cipher process */
     ...
     *  The erased data should be 0xFFFFFFFF
     */
    ...
    {
        memset(gu8Buff, 0, BUFFER_SIZE);

#if (NVT_DCACHE_ON == 1)
        // Invalidate the data cache for the buffer to ensure data consistency
        SCB_InvalidateDCache_by_Addr((uint32_t *)&gu8Buff, sizeof(gu8Buff));
#endif

        SPIM_IO_Read(SPIM_PORT, TEST_BLOCK_ADDR + u32Offset, is4ByteAddr,
                     BUFFER_SIZE, gu8Buff, OPCODE_FAST_READ,
                     SPIM_BITMODE_1, SPIM_BITMODE_1, SPIM_BITMODE_1,
                     8);

        pu32Data = (uint32_t *)gu8Buff;
    ...
    for (u32Offset = 0; u32Offset < FLASH_BLOCK_SIZE; u32Offset += BUFFER_SIZE)
    {
        ...
        SCB_CleanDCache_by_Addr((uint32_t *)&gu8Buff, sizeof(gu8Buff));
#endif
        /*
         *  Program data to flash block with DMA
         */
        SPIM_DMA_Write(SPIM_PORT, TEST_BLOCK_ADDR + u32Offset, is4ByteAddr, BUFFER_SIZE, gu8Buff, WrCmd);
    }
    ...
    /*
     *  Verify the data read back from SPI flash
     */
    for (u32Offset = 0; u32Offset < FLASH_BLOCK_SIZE; u32Offset += BUFFER_SIZE)
    {
        memset(gu8Buff, 0, BUFFER_SIZE);
        ...
        SCB_InvalidateDCache_by_Addr((uint32_t *)&gu8Buff, sizeof(gu8Buff));
#endif
        SPIM_DMA_Read(SPIM_PORT, TEST_BLOCK_ADDR + u32Offset, is4ByteAddr,
                      BUFFER_SIZE, gu8Buff, u32RdCmd, SPIM_OP_ENABLE);
        ...
        {
            /*
             *  Verify data read from SPI flash
             */
            if ((*pu32Data) != ((u32i << 16) | (TEST_BLOCK_ADDR + u32Offset + u32i)))
            {
                ...
                printf("Flash address 0x%x, read 0x%x, expect 0x%x!\n",
                       TEST_BLOCK_ADDR + u32i,
                       *pu32Data,
                       (u32i << 16) | (TEST_BLOCK_ADDR + u32Offset + u32i));
                return SPIM_ERR_FAIL;
            }
        }
    }
     ...
     *  Note: SPIM_IO_Read() uses OPCODE_FAST_READ to read encrypted data from SPI flash.
     */
    memset(gu8Buff, 0, VERIFY_SIZE);
    printf("\n\nDump SPI flash with I/O read. It's the SPIM cipher encrypted text.\n");
    ...
    SPIM_IO_Read(SPIM_PORT, TEST_BLOCK_ADDR, is4ByteAddr,
                 VERIFY_SIZE, gu8Buff, OPCODE_FAST_READ,
                 SPIM_BITMODE_1, SPIM_BITMODE_1, SPIM_BITMODE_1,
                 8);
    DumpHex(gu8Buff, VERIFY_SIZE);
    ...
    SPIM_DMA_Read(SPIM_PORT, TEST_BLOCK_ADDR, is4ByteAddr, VERIFY_SIZE, gu8Buff, u32RdCmd, SPIM_OP_ENABLE);
    DumpHex(gu8Buff, VERIFY_SIZE);
