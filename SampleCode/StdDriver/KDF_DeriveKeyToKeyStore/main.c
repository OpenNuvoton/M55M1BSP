...
void DumpKeyFromKS(int32_t i32KeyIdx, uint32_t u32WordCnt)
{
    int32_t i;

    memset(g_au32Keyout, 0x0, sizeof(g_au32Keyout));

    if (KS_Read(KS_SRAM, i32KeyIdx, g_au32Keyout, u32WordCnt) < 0)
    {
...
int main(void)
{
    ...
    if ((i32KeyIdx = KDF_DeriveKeyToKS(KS_SRAM, eKDF_MODE_HKDF, (KDF_KEYIN_FROM_REG | KDF_SALT_FROM_RANDOM | KDF_LABEL_FROM_RANDOM | KDF_CONTEXT_FROM_RANDOM), KDF_KS_KEYSIZE_256, KDF_KS_OWNER_CPU)) < 0)
    {
        printf("Failed to derive key to KS !\n");
        goto Error_Exit;
    }
