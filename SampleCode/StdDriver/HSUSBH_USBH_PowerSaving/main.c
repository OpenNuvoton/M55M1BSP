 ...
 * @file     main.c
 * @version  V1.00
 * @brief    This sample uses power down to do power saving with usb host.
 *           System wake up after, user need restore and reset usb host.
 *           It also shows device information represented in UDEV_T.
 *
...
#define USB_DEVICE_DISCONNECT       0
#define USB_PORT20                  20
#define USB_PORT11                  11

static volatile uint32_t s_u32TickCnt;
uint32_t u32Device_Connect = USB_DEVICE_DISCONNECT; // 0 : dis-connect, 1 : connect
uint32_t u32Usb_Port = 0;
...
void connect_func(struct udev_t *udev, int i8Param)
{
    ...
    u32Device_Connect = USB_DEVICE_CONNECT;

    if (parent == NULL)
    {
        if (udev->port_num == 1)
        {
            u32Usb_Port = USB_PORT20;
            printf("USB 2.0 port\n");
        }
        else
        {
            u32Usb_Port = USB_PORT11;
            printf("USB 1.1 port\n");
        }
    }
    else
...
void disconnect_func(struct udev_t *udev, int i8Param)
{
    ...
    u32Device_Connect = USB_DEVICE_DISCONNECT;
    u32Usb_Port = 0;
...
int32_t main(void)
{
    ...
    printf("+--------------------------------------------+\n");
    printf("|                                            |\n");
    printf("| USB Host Power Saving demo                 |\n");
    printf("|                                            |\n");
    printf("+--------------------------------------------+\n");
    printf("USB 2.0 port has not support power down mode wake up\n");
    printf("user need to use else wake up source to wake up system, etc.gpio\n");
    printf("USB 1.1 port has support power down mode wake up\n");
    ...
    while (1)
    {
        if (usbh_pooling_hubs())             /* USB Host port detect polling and management */
        {
            ...
            if (u32Usb_Port == USB_PORT11)
            {
                printf("PORT1.1 demo\n");
                /* Enter to Power-down mode */
                printf("Enter to Power-Down. using disconnect/connect device or remote wake to wake up system...\n");

                usbh_suspend();

                delay_us(100000);
                printf("Enter to Power-Down and supend usbh port ...\n");
                //into power down
                PowerDownFunction();
                ...
                printf("\nEnter to Power-Down and supend usbh port ...\n");
                ...
                printf("finish demo\n");
                printf("wait disconnect or connect device .....\n\n");
            }
            else if (u32Usb_Port == USB_PORT20)
            {
                printf("PORT2.0 demo\n");
            ...
            else
            {
                ...
                printf("Please, connect device again\n");
            }
        }
    }
}
