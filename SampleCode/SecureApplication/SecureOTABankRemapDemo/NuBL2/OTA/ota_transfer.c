...
#define LED_OFF         PG15        // Green LED

#if defined( NUMAKER_BOARD )
#define RST_PIN         PD2
//#define FW_UPDATE_OFF   PD12      // No dedicated pin in NUMAKER_BOARD
#define IOCTL_INIT      { \
        CLK_EnableModuleClock(GPIOG_MODULE); \
        CLK_EnableModuleClock(GPIOD_MODULE); \
        PG->MODE = (GPIO_MODE_OUTPUT << (15 * 2)); \
        PD->MODE = (GPIO_MODE_OUTPUT << ( 2 * 2)); \
    }
#endif
...
void Transfer_WiFiProcess(void)
{
    if (UART_GET_INT_FLAG(WIFI_PORT, UART_INTSTS_RDAINT_Msk))
    {
        while (UART_IS_RX_READY(WIFI_PORT))
        {
            ...
            WiFi_ReceiveHandler((uint8_t)UART_READ(WIFI_PORT));
        }
    }

    if (UART_GET_INT_FLAG(WIFI_PORT, UART_INTSTS_BUFERRIF_Msk))
    {
        WIFI_PORT->FIFOSTS |= (UART_FIFOSTS_RXOVIF_Msk | UART_FIFOSTS_TXOVIF_Msk);
    }
}

/*----------------------------------------------------------------------------
  UART8 IRQ Handler(Wi-Fi module)
 *----------------------------------------------------------------------------*/
void UART8_IRQHandler(void)
{
...
/* Init UART8 for Wi-Fi module communication */
void WiFi_UART_Init(void)
{
