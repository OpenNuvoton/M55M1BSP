    ...
    switch (((CMD_PACKET_T *)s_psISPInfo->rcvbuf)->u16CmdID)
    {
        ...
        case CMD_RESET: //need ??(CHIP reset?System reset?CPU reset)
            ...
            if (i32Ret == CMD_SET_MASS_WRITE)
            {
                ...
                {
                    g_u32NewFwStartAddr += (s_psISPInfo->u32UpdateBankIdx * FMC_APROM_BANK_SIZE);

                    for (u32Addr = g_u32NewFwStartAddr; u32Addr < (g_u32NewFwStartAddr + g_u32NewFwByteSize); u32Addr += FMC_FLASH_PAGE_SIZE)
