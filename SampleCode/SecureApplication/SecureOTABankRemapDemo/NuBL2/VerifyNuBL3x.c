...
/* Public key used to verfiy NuBL32 and NuBL33 signed image. */
const char gc_strNuBL3x_PubKey0[ECDSA_EC256_KEY_LEN * 2 + 1] = "18ab8717ffe2bb2545238176bc3bd43d49fa036759979994a4eacbfe7be1c620";
const char gc_strNuBL3x_PubKey1[ECDSA_EC256_KEY_LEN * 2 + 1] = "724cb73609d0106a325b570016310f9b10c9738b5ce489a0efac88f2062d58da";
...
int32_t NuBL_VerifyNuBL3x(int32_t i32BootBank, uint32_t u32ImgBaseAddr, uint32_t u32ImgByteSize, uint32_t *pu32ImgStartAddr)
{
    struct image_header     *psImgHdr = (struct image_header *)(u32ImgBaseAddr + (i32BootBank * FMC_APROM_BANK_SIZE));
    ...
    if (Cal_SHA256_Flash(u32ImgBaseAddr + (i32BootBank * FMC_APROM_BANK_SIZE), psImgHdr->ih_hdr_size + psImgHdr->ih_img_size + psImgHdr->ih_protect_tlv_size, au32ImgHash) != 0)
        return eBOOT_ERRCODE_BADHASH;

    psTlvInfo = (struct image_tlv_info *)(u32ImgBaseAddr + (i32BootBank * FMC_APROM_BANK_SIZE) + BOOT_TLV_OFF(psImgHdr));
    ...
    while ((uint32_t)psTlv < ((uint32_t)psTlvInfo + psTlvInfo->it_tlv_tot))
    {
        ...
        else if (psTlv->it_type == IMAGE_TLV_ECDSA_SIG)
        {
            ...
            */
            ...
            /* Verify signature */
            if (ECC_VerifySignature(CRYPTO, ecc_curve, strDigest, (char *)gc_strNuBL3x_PubKey0, (char *)gc_strNuBL3x_PubKey1, R, S) < 0)
            {
