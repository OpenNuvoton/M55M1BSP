...
#if (NVT_DCACHE_ON == 1)
// Data cache-line aligned buffer for SPIM0 data transfer with DMA mode
uint32_t g_au32ReadBuf[DCACHE_ALIGN_LINE_SIZE(SPI_FLASH_PAGE_SIZE / 4)] __ALIGNED(DCACHE_LINE_SIZE) = { 0 };
#else
// 32-byte aligned buffer for SPIM0 data transfer with DMA mode when DCache is disabled
uint32_t g_au32ReadBuf[SPI_FLASH_PAGE_SIZE / 4] __ALIGNED(32) = { 0 };
#endif
...
static uint16_t PACKET_ExecCCITT(uint32_t *pu32buf, uint16_t len, uint8_t mode)
{
    ...
    {
        NUBL_MSG("OrgSum: 0x%08X, CalSum: 0x%08X\n", OrgSum, CalSum);
...
uint32_t GetChkSum(uint32_t u32StartAddr, uint32_t u32ByteSize)
{
        ...
        else
        {
            ...
            switch (cmd_case)
            {
                ...
                case CMD_CONNECT:
                    ...
                    DEBUG_MSG("PDID:  0x%08x.\n", cmd.au32Data[1]);

                    for (i = 0; i < 4; i++)
                        DEBUG_MSG("Target IV[%d]: 0x%08x.\n", i, pISPInfo->au32AESIV[i]);
...
static int32_t GenRandomPrivKey(char *d, int32_t nbits)
{
    ...
    do
    {
        ...
        {
            /* Invalid key */
            ERROR_MSG("Current private key is not valid. Need a new one.\n");
            return -1;
        }
    } while (1);
}
...
int32_t ParseECDH(ISP_INFO_T *pISPInfo)
{
        ...
        else
        {
            ...
            switch (cmd_case)
            {
                ...
                case CMD_ECDH_RAND_PUB0: // MUST be encrypted
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        DEBUG_MSG("Get rand_pub0[%d]: 0x%08x.\n", i, pISPInfo->ServerPubKey.au32Key0[i]);
                    }
                ...
                case CMD_ECDH_RAND_PUB1: // MUST be encrypted
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        DEBUG_MSG("Get rand_pub1[%d]: 0x%08x.\n", i, pISPInfo->ServerPubKey.au32Key1[i]);
                    }
                ...
                case CMD_ECDH_GET_RAND_PUB0: // MUST be encrypted
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        DEBUG_MSG("Out rand_pub0[%d]: 0x%08x.\n", i, cmd.au32Data[1 + i]);
                    }
                ...
                case CMD_ECDH_GET_RAND_PUB1: // MUST be encrypted
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        DEBUG_MSG("Out rand_pub1[%d]: 0x%08x.\n", i, cmd.au32Data[1 + i]);
                    }
    ...
    if ((ret == CMD_ECDH_PUB1) || (ret == CMD_ECDH_GET_RAND_PUB1))
    {
        if (ret == CMD_ECDH_PUB1)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                DEBUG_MSG("Gen 1st ECDH AES KEY[%d]: 0x%08x.\n", i, pISPInfo->au32AESKey[i]);
            }
        }
        else
        {
            ...
            for (i = 0; i < 8; i++)
            {
                DEBUG_MSG("Gen 2nd ECDH AES KEY[%d]: 0x%08x.\n", i, pISPInfo->au32AESKey[i]);
            }
...
int32_t _PageErase(uint32_t u32Addr, uint32_t count, uint32_t u32CmdMask)
{
    volatile int32_t    i, ret = -1;

    DEBUG_MSG("[Page erase] address: 0x%x, page counts: %d.\n", u32Addr, count);

    u32Addr &= ~NS_OFFSET;

    /* Check address and counts */
    if (((u32Addr % SPI_FLASH_PAGE_SIZE) != 0) || (count == 0))
        return ERR_PAGE_ALIGN;

    if (u32Addr < SPIFlash_GetFlashSize(SPIM_PORT))
    {
        if (count > (SPIFlash_GetFlashSize(SPIM_PORT) / SPI_FLASH_PAGE_SIZE))
            return ERR_OVER_RANGE;

        if ((u32Addr + (count * SPI_FLASH_PAGE_SIZE)) > SPIFlash_GetFlashSize(SPIM_PORT))
            return ERR_OVER_RANGE;
    }
    else
    {
    ...
    for (i = 0; i < count; i++)
    {
        ...
        /* perform erase page */
        if (SPIFlash_ErasePage(SPIM_PORT, u32Addr) < 0)
            break;

        /* run 2K checksum and verify its result */
        if (GetChkSum(u32Addr, SPI_FLASH_PAGE_SIZE) != ALL_ONE_2KB_CHKS)
            break;

        u32Addr += SPI_FLASH_PAGE_SIZE;
        ret = 0;
    }
...
int32_t _WriteFlash(uint32_t u32Addr, uint32_t u32ByteSize, uint32_t *pu32Data, uint32_t u32CmdMask)
{
    int32_t  i, i32RetCode = -1;
    uint32_t u32StartAddr = u32Addr,
             u32Offset;

    DEBUG_MSG("[Write flash] address: 0x%x, size: %d.\n", u32StartAddr, u32ByteSize);

    if ((i32RetCode = _IsValidFlashRegion(u32StartAddr, u32ByteSize)) != 0)
        return i32RetCode;

    u32StartAddr &= ~NS_OFFSET;

    for (u32Offset = 0; u32Offset < u32ByteSize; u32Offset += SPI_FLASH_PAGE_SIZE)
    {
        DEBUG_MSG("Program offset: 0x%08X\n", u32StartAddr + u32Offset);

        /*
         *  Program data to flash block with DMA
         */
#if (NVT_DCACHE_ON == 1)
        // Clean the data cache for the buffer to ensure data consistency
        SCB_CleanDCache_by_Addr(pu32Data, u32ByteSize);
#endif

        /* write */
        SPIFlash_WritePage(SPIM_PORT, u32StartAddr + u32Offset, pu32Data, SPI_FLASH_PAGE_SIZE);

        memset(g_au32ReadBuf, 0, SPI_FLASH_PAGE_SIZE);
#if (NVT_DCACHE_ON == 1)
        // Invalidate the data cache for the buffer to ensure data consistency
        // before reading from SPI flash
        SCB_InvalidateDCache_by_Addr(g_au32ReadBuf, SPI_FLASH_PAGE_SIZE);
#endif
        /* verify */
        SPIFlash_ReadPage(SPIM_PORT, u32StartAddr + u32Offset, g_au32ReadBuf, SPI_FLASH_PAGE_SIZE);

        for (i = 0; i < (SPI_FLASH_PAGE_SIZE / 4); i++)
        {
            if (g_au32ReadBuf[i] != pu32Data[i])
            {
                ERROR_MSG("Verify 0x%08X failed ! (0x%08X != 0x%08X)\n", u32StartAddr + u32Offset + i, g_au32ReadBuf[i], pu32Data[i]);
                i32RetCode = ERR_ISP_WRITE;
                break;
            }
