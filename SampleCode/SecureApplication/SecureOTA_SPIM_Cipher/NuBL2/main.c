...
/* User defined SPIM cipher key */
const uint32_t gc_au32SPIM_CipherKey[8] =
{
    0x93484D6F, // Key0
    0x2F7A7F2A, // Key1
    0x063FF08A, // Key2
...
int32_t _IsValidFlashRegion(uint32_t u32Addr, uint32_t u32ByteSize, uint32_t u32CmdMask)
{
    ...
    if (u32ByteSize > SPIFlash_GetFlashSize(SPIM_PORT))
    {
...
void SYS_Init(void)
{
    ...
    GPIO_ENABLE_SCHMITT_TRIGGER(PH, BIT13);
    GPIO_ENABLE_SCHMITT_TRIGGER(PJ, BIT3 | BIT4 | BIT5 | BIT6 | BIT7);

    /* Set SPIM I/O pins as high slew rate up to 80 MHz. */
    GPIO_SetSlewCtl(PH, BIT13, GPIO_SLEWCTL_HIGH);

    GPIO_SetSlewCtl(PJ, BIT3 | BIT4 | BIT5 | BIT6 | BIT7, GPIO_SLEWCTL_HIGH);
...
PFN_FUNC pfnSPIM = (PFN_FUNC)(SPIM_HYPER_DMM0_SADDR + 1);
int32_t SPIM_WriteImage(void);
/*
 *       APROM Layout              HyperFlash Layout
 * +---- 0x0010_0000 ----+   |-> +---- 0x8200_0000 ----+
 * |                     |   |   |                     |
 * |        NuBL2        | --|   |       NuBL32        |
 * |   (Secure Region)   |       |   (Secure Region)   |
 * |                     |       |                     |
 * |---- 0x001A_0000 ----|       |---- 0x8210_0000 ----|
 * |                     |       |                     |
 * |  (NonSecure Region) |       |       NuBL33        |
 * |                     |       |  (NonSecure Region) |
 * +---- 0x001F_FFFF ----+       |                     |
 *                               +---- 0x83FF_FFFF ----+
 */
...
int main(void)
{
    ...
    printf("+----------------------------------+\n");
    printf("| M55M1 SecureOTA_SPIM_Cipher      |\n");
    ...
    OTFC_SetKeyFromKeyReg(OTFC0, (uint32_t *)gc_au32SPIM_CipherKey, OTFC_PR_0, 0, SPIM_HYPER_DMM_SIZE);
    ...
    {
        //SPIFlash_EraseBlock(SPIM_PORT, NUBL32_FW_BASE - SPIM_DMM0_SADDR);
    ...
    {
        //SPIFlash_EraseBlock(SPIM_PORT, (NUBL33_FW_BASE & ~NS_OFFSET) - SPIM_DMM0_SADDR);
...
OTA_PROCESS:
    SYS_UnlockReg();
    // Set read mode to SPIM_CTL0_OPMODE_PAGEREAD to use SPIFlash_ReadPage
    SPIFlash_SetReadMode(SPIM_CTL0_OPMODE_PAGEREAD);
