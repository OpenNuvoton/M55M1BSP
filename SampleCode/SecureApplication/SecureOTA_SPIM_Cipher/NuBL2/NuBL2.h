...
#include "../image.h"
    ...
    #define NUBL_MSG    printf
    #define DEBUG_MSG   printf
#endif

// For Windows use '\\' instead of '/'.
#define __FILENAME__ (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)
#define ERROR_MSG(fmt, ...)     printf("[Error][%s:%d] " fmt, __FILENAME__, __LINE__, ##__VA_ARGS__)

/*---------------------------------------------------------------------------------------------------------*/
/* FLASH memory layout definitions                                                                         */
/*---------------------------------------------------------------------------------------------------------*/
#define NUBL2_FW_BASE           (FMC_APROM_BASE + 0x00000000ul)     /* Secure code */
#define NUBL32_FW_BASE          (SPIM_DMM0_SADDR)                   /* Secure code */
#define NUBL32_FW_SIZE          (0x100000ul)
#define NUBL33_FW_BASE          (SPIM_DMM0_NSADDR + NUBL32_FW_SIZE) /* Non-secure code start address */
#define NUBL33_FW_SIZE          (0x100000ul)
