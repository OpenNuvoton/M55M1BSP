...
{
    return TVMPlatformMemoryFree(ptr, dev);
}
...
{
    return TVMMutableFuncRegistry_Set(&global_func_registry, name, (TVMBackendPackedCFunc)f, override != 0);
}
...
tvm_crt_error_t TVMInitializeRuntime()
{
    ...
    if (error == kTvmErrorNoError)
    {
        error = (tvm_crt_error_t)TVMFuncRegisterGlobal("runtime.SystemLib", (TVMFunctionHandle)&SystemLibraryCreate, 0);
    }

    if (error == kTvmErrorNoError)
    {
        error = (tvm_crt_error_t)TVMFuncRegisterGlobal("tvm.rpc.server.ModuleGetFunction", (TVMFunctionHandle)&ModuleGetFunction, 0);
    }

    if (error == kTvmErrorNoError)
    {
        error = (tvm_crt_error_t)TVMFuncRegisterGlobal("runtime.RPCTimeEvaluator", (TVMFunctionHandle)&RPCTimeEvaluator, 0);
    }

    if (error == kTvmErrorNoError)
    {
        error = (tvm_crt_error_t)TVMFuncRegisterGlobal("tvm.rpc.server.GetCRTMaxPacketSize", (TVMFunctionHandle)&RPCGetCRTMaxPacketSize, 0);
    }

    if (error == kTvmErrorNoError)
    {
        error = (tvm_crt_error_t)TVMFuncRegisterGlobal("tvm.contrib.random.random_fill", (TVMFunctionHandle)&RandomFill, 0);
    }
...
// Default implementation, overridden by the platform runtime.
TVM_WEAK tvm_crt_error_t TVMPlatformBeforeMeasurement()
{
    return kTvmErrorNoError;
}

// Default implementation, overridden by the platform runtime.
TVM_WEAK tvm_crt_error_t TVMPlatformAfterMeasurement()
{
    return kTvmErrorNoError;
}
