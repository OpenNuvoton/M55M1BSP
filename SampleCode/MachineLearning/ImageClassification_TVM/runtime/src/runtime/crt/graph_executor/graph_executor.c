                                ...
                                const uint32_t param_size)
{
    ...
    for (idx = 0; idx < size; idx++)
    {
        ...
        {
            fprintf(stderr, "`entry_id`=%u is greater than expected(%d).\n", eid,
        ...
        printf("loading: param %s loaded, in_idx=%d, eid=%u, ndim=%d, data[0]=%f\n",
...
int TVMGraphExecutor_SetupOpExecs(TVMGraphExecutor *executor)
{
    ...
    for (nid = 0; nid < executor->nodes_count; nid++)
    {
        ...
        if (strcmp(inode->op_type, "null"))
        {
            ...
            {
                fprintf(stderr, "too many arguments: expected less than %d args, but got %u.\n",
...
#if TVM_CRT_DEBUG
            printf("tvm_op: creating %s with node_id=%u\n", inode->param.func_name, nid);
#endif  // TVM_CRT_DEBUG
            TVMPackedFunc pf;
