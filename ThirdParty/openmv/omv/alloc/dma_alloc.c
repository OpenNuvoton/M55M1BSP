...
//            MP_ERROR_TEXT("DMA buffer allocation failed while allocating %u bytes"), (uint) size);
    printf("DMA buffer allocation failed while allocating %u bytes \n", size);
}
...
//                MP_ERROR_TEXT("DMA buffer allocation failed. No DMA buffer reserved for peripheral 0x%x"), (uint) periph);
            printf("DMA buffer allocation failed. No DMA buffer reserved for peripheral 0x%x \n", (uint32_t)periph);
            break;
    }
}
...
//Modify by chchen59
            ...
            uint8_t *block_end = (uint8_t *)BLOCK_TO_PTR(block) + block->size;
            uint32_t freemem = (uint8_t *)block_chain_end - (uint8_t *)block_end;
#else
...
//    block->used = true;     // Mark block as used.
    block->used = 1;     // Mark block as used.
    block->periph = periph;
    if (block->next == NULL) {
