...
void fft2d_alloc(fft2d_controller_t *controller, image_t *img, rectangle_t *r)
{
    ...
    if (!rectangle_subimg(controller->img, r, &controller->r)) {
		//Modify by chchen59
        //mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("lodepng_decode"));
		printf("lodepng_decode \n");
    }
...
//Modify by chchen59
              ...
              tmp[j] = COLOR_RGB565_TO_Y(IM_GET_RGB565_PIXEL(controller->img,
                    controller->r.x + j, controller->r.y + i));
#else
              uint16_t tmp16 = IM_GET_RGB565_PIXEL(controller->img, controller->r.x + j, controller->r.y + i);
              tmp[j] = COLOR_RGB565_TO_Y(tmp16);
#endif
            }
        }
