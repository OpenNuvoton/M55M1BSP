...
void* imlib_compute_row_ptr(const image_t *img, int y) {
    switch (img->pixfmt) {
        case PIXFORMAT_BINARY: {
            return IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR((image_t *)img, y);
        }
        case PIXFORMAT_GRAYSCALE: {
            return IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR((image_t *)img, y);
        }
        case PIXFORMAT_RGB565: {
            return IMAGE_COMPUTE_RGB565_PIXEL_ROW_PTR((image_t *)img, y);
        }
        default: {
...
//Modify by chchen59
    ...
    ({ \
        ...
        uint32_t rgb = (src_pixel << 16) | dst_pixel; \
...
//Modify by chchen59
    ...
    ({ \
        int rb_out = ((src_pixel & 0xf81f) * smuad_alpha) >> 5; \
        int g_out = ((src_pixel & 0x7e0) * smuad_alpha) >> 5; \
        ...
        const uint8_t *alpha_palette, image_hint_t hint, imlib_draw_row_callback_t callback, void *dst_row_override)
{
    ...
    imlib_draw_row_data.callback = (void *)callback;
